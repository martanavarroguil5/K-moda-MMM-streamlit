...
HISTORICAL_12M = {
    "paid_search": 2_715_279.965590818,
    "email_crm": 587_799.0130643218,
    "display": 939_275.4976916444,
    "prensa": 1_074_233.6738461927,
    "video_online": 1_869_730.2118123923,
    "social_paid": 2_085_731.9776999382,
    "exterior": 1_431_691.962483214,
    "radio_local": 1_296_257.6978114776,
}
...
def fallback_weights_frame() -> pd.DataFrame:
    df = pd.DataFrame(
        [{"channel": channel, "historical_budget_eur": HISTORICAL_12M[channel]} for channel in CHANNELS]
    )
    total = float(df["historical_budget_eur"].sum())
    df["historical_share_pct"] = df["historical_budget_eur"] / total * 100.0
    df["observed_budget_2024_eur"] = df["historical_budget_eur"]
    return df


def prepare_weights(weights: pd.DataFrame) -> pd.DataFrame:
    if weights is None or weights.empty or "channel" not in weights.columns:
        return fallback_weights_frame()

    df = weights.copy()
    if "historical_budget_eur" not in df.columns:
        df["historical_budget_eur"] = df["channel"].map(HISTORICAL_12M)

    if "historical_share_pct" not in df.columns:
        df["historical_share_pct"] = np.nan

    df = df[df["channel"].isin(CHANNELS)].copy()
    existing = set(df["channel"].dropna().astype(str))
    ...
    if missing:
        df = pd.concat(
            [
                df,
                pd.DataFrame(
                    [{"channel": channel, "historical_budget_eur": HISTORICAL_12M[channel]} for channel in missing]
                ),
            ],
            ignore_index=True,
        )

    df["historical_budget_eur"] = pd.to_numeric(df["historical_budget_eur"], errors="coerce")
    df["historical_budget_eur"] = df["historical_budget_eur"].fillna(df["channel"].map(HISTORICAL_12M))

    df["historical_share_pct"] = pd.to_numeric(df["historical_share_pct"], errors="coerce")
    total = float(df["historical_budget_eur"].sum())
    if total > 0:
        computed_share = df["historical_budget_eur"] / total * 100.0
        df["historical_share_pct"] = df["historical_share_pct"].fillna(computed_share)

    if "observed_budget_2024_eur" not in df.columns:
        df["observed_budget_2024_eur"] = df["historical_budget_eur"]

    order = pd.Categorical(df["channel"], categories=CHANNELS, ordered=True)
    return df.sort_values("channel", key=lambda _: order).reset_index(drop=True)
